...
My dtb which is packed in a fitimage is now suddenly being unpacked to a bogus location when a kernel version is being bumped. This intepretation was bolstered by the fact NFS booting the kernel with explicit device loading was functional. I assumed some secondary mechanism had previously been making this work, despite a malconfigured (turns out an actually absent) loadaddr. Introducing an explicit loadaddr to the dtb entry in fitimage config did acually end up resolving the issue and the kernel proceeded to boot.
